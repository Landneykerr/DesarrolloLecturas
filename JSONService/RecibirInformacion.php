            ...
            if($request['informacion'][$i]['anomalia']==22||$request['informacion'][$i]['anomalia']==52){
                if(count($request['informacion'][$i]['fotos'])>0){
                    for($j=0;$j<count($request['informacion'][$i]['fotos']);$j++){          
                        ...
                        $queryRespuestaFoto = $postgresWS->PostgresFunction("toma.recibir_lectura_foto(".$request['informacion'][$i]['id_serial1'].",".$request['informacion'][$i]['fotos'][$j]['cuenta'].",'".$request['informacion'][$i]['fotos'][$j]['nombre_foto']."','".$request['informacion'][$i]['fotos'][$j]['fecha_toma']."','".$imdata."')");
