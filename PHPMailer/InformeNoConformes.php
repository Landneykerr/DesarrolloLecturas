<?php
	...
	require_once ('/media/sda1/www/ControlEnergia/jpgraph/jpgraph_pie3d.php');
		...
		$csv_end = "\n";  
		$csv_sep = ";";
		$csv_file 	= 	'Informe No Conformes '.$fecha.'.csv';
		$graph_file = 	'Grafica Informe No Conformidades '.$fecha.'.png';
		$csv 		=	"PDA;TECNICO;REGISTRADAS;SIN ANALIZAR;CONFORMES;NO CONFORMES\n";

		$Conexion = DoConection();
        $Query = pg_query($Conexion,"SELECT * FROM ordenes_trabajo_historico.InformeNoConformidades('".$fecha."')");
        ...
        while($RtaQuery = pg_fetch_assoc($Query)){
        	if($RtaQuery['tecnico']=='RESUMEN TOTALES'){
        		$registradas 	= $RtaQuery['registradas'];
        		$sin_analizar 	= $RtaQuery['sin_analizar'];
        		$conformes 		= $RtaQuery['conformes'];
        		$no_conformes 	= $RtaQuery['no_conformes'];
        	}
        	$csv.=$RtaQuery['pda'].$csv_sep.$RtaQuery['tecnico'].$csv_sep.$RtaQuery['registradas'].$csv_sep.$RtaQuery['sin_analizar'].$csv_sep.$RtaQuery['conformes'].$csv_sep.$RtaQuery['no_conformes'].$csv_end;
        }
        pg_close($Conexion);   
		...
		$graph->SetShadow();		 
		$graph->title->Set("Resumen Informe De No Conformidades ".$fecha);		 
		...
		$mail             = new PHPMailer();
		$body             = "Buen dia. \n Se adjunta archivo .csv y grafica con informe de no conformidades de actas a la fecha ".$fecha.", este informe es realizado de forma automatica por el sistema y se basa exclusivamente en la informacion recibida en linea por las cuadrillas, favor no responder este correo.";
		...
		$mail->Subject    = "Informe De No Conformidades ".$fecha;
		$mail->MsgHTML($body);
		...
		$mail->AddAttachment('Grafica Informe No Conformidades '.$fecha.'.png'); 
