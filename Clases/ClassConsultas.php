<?php
	...
	class Consultas{
        ...
        function ConsultaRutasCiclo($_mes, $_anno, $_ciclos){
			...
			return json_encode($this->consulta_connect->QueryToJson($Informacion,["ruta","inspector","total","leidas","pendientes","estado","municipio"],[null,null,null,null,null,null],false));
			$this->consulta_connect->ClosePostgres();
        }
		...
		function ConsultaLecturasTomadas($_mes, $_anno, $_inspectores){
			...
			$ClientesLeidos 	= $this->consulta_connect->QueryToJson($Informacion,
					["ruta","cuenta","medidor","nombre","direccion","str_lectura","fecha_toma","fecha_recepcion","descripcion_anomalia","mensaje","descripcion_critica","id_lector","str_gps","str_magna"],
		...
		function CuentaPosterior($_cuenta){
        	$this->consulta_connect->OpenPostgres();
        	$Informacion = 	$this->consulta_connect->PostgresFunctionCamposTable("id_maestro,cuenta,medidor,nombre,direccion,fecha_toma,lector",
        																		"reportes.cuenta_ubicacion(".$_cuenta.")");
			return $this->consulta_connect->QueryToJson($Informacion,["id_maestro","cuenta","medidor","nombre","direccion","fecha_toma","lector"],[null,null,null,null,null,null,null],false);
			$this->consulta_connect->ClosePostgres();
		}	
        ...
        **/
        ...
        function consultarRutaCuentaDatos($mes, $anno, $ciclo, $municipio, $ruta){
        	...
        	$this->consulta_connect->setConexion('fotos');
			$this->consulta_connect->OpenPostgres();
			...
			$i = 0;
			while ($row = pg_fetch_assoc($Informacion)) {
				$query =  $this->consulta_connect->PostgresSelectJoinWhereOrder("registro.cuentas AS a", 
																	"b.foto", 
																	"registro.fotos AS b", 
																	"a.id_serial = b.id_cuenta",
																	"a.cuenta =".$row['cuenta']." AND a.mes =".$mes." AND a.anno=".$anno." AND a.id_ciclo=".$ciclo."", 
																	"a.cuenta");
  			...
  			$this->consulta_connect->ClosePostgres();	
			$this->consulta_connect->setDefaultConexion();
			$this->consulta_connect->OpenPostgres();
